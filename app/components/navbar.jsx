...
function NavLinkSm({ href, text, onClick }) {
  return (
    <a className="hover:text-main-medium transition-all delay-100 text-xl pb-2 pt-2" href={href} onClick={onClick}>
      {text}
    </a>
  );
}
...
function Navbar() {
  ...
  return (
    <nav className="flex justify-between px-4 py-2 w-full relative">
      ...
      <button type="button" onClick={() => setOpen(!open)} className="lg:hidden relative z-10 text-[2.2rem] text-main-medium drop-shadow-sm m-3">
        <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]" ref={refOpen}>
          <SlMenu />
        </div>
        <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] closed" ref={refClose}>
          <TfiClose />
        </div>
      </button>
      {/* Nav for mobile */}
      <div id="nav-menu" className="lg:hidden justify-center absolute pt-12 flex items-center w-[100%] left-[50%] translate-x-[-50%]">
        <div className={`flex flex-col w-full items-center bg-white-medium ${open ? 'down' : 'up'}`}>
          <NavLinkSm href="/" text="Home" onClick={() => setOpen(false)} />
          <hr className="w-[80%] text-black-light" />
          <NavLinkSm href="/about" text="About" onClick={() => setOpen(false)} />
          <hr className="w-[80%] text-black-light" />
          <NavLinkSm href="/contact" text="Contact" onClick={() => setOpen(false)} />
          <hr className="w-[80%] text-black-light pb-2" />
          <BtnInvSm><a href="/#quote" onClick={() => setOpen(false)}>Get Started</a></BtnInvSm>
        </div>
      </div>
